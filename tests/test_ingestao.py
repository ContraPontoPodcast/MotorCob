...
LAYOUT = {
    ...
    "colunas": {"cpf": "CPF", "contato": "FONE", "data": "DATA", "resultado": "COD", "id_externo": "ID", "custo": "CUSTO"},
...
class TestLeitura(unittest.TestCase):
    ...
    def escrever(self, nome, linhas):
        p = self.pasta / nome
        p.write_text("CPF;FONE;DATA;COD;ID;CUSTO\n" + "\n".join(linhas) + "\n", encoding="utf-8")
        return p

    def test_le_normaliza_e_classifica(self):
        p = self.escrever("teste_1.csv", [f"{CPF};(11) 99999-0000;20/09/2026;1;A;0,35"])
        eventos, rel, q = ler_retorno(p, validar_layout(LAYOUT))
        self.assertEqual(rel.aceitas, 1)
        e = eventos[0]
        self.assertEqual((e.cpf, e.contato, e.resultado, e.data, e.custo),
                         (CPF, "11999990000", "cpc", date(2026, 9, 20), 0.35))

    def test_codigo_desconhecido_vai_para_quarentena_mascarado(self):
        p = self.escrever("teste_1.csv", [f"{CPF};11999990000;20/09/2026;77;A;0,35"])
        eventos, rel, q = ler_retorno(p, validar_layout(LAYOUT))
    ...
    def test_rejeicoes_e_duplicadas(self):
        p = self.escrever("teste_1.csv", [
            f"{CPF};11999990000;20/09/2026;1;A;0,35",
            f"{CPF};11999990000;20/09/2026;1;A;0,35",   # reexportação
            f"12345678900;11999990000;20/09/2026;1;B;0,35",  # CPF inválido
            f"{CPF};123;20/09/2026;1;C;0,35",           # telefone inválido
            f"{CPF};11999990000;2026-09-20;1;D;0,35",   # data fora do formato
        ])
        eventos, rel, _ = ler_retorno(p, validar_layout(LAYOUT))
        self.assertEqual((rel.aceitas, rel.duplicadas), (1, 1))
        self.assertEqual(rel.rejeitadas, {"cpf inválido": 1, "contato inválido": 1, "data inválida": 1})

    def test_arquivo_sem_layout_nao_e_processado(self):
        self.escrever("teste_1.csv", [f"{CPF};11999990000;20/09/2026;1;A;0,35"])
    ...
    def test_coluna_ausente_no_arquivo_falha_alto(self):
        ...
        p.write_text("CPF;FONE\n1;2\n")
        with self.assertRaises(LayoutInvalido):
            ler_retorno(p, validar_layout(LAYOUT))

    def test_carteira_normaliza_e_deduplica(self):
        p = self.pasta / "carteira.csv"
        p.write_text(f"cpf;contato;tipo;origem\n{CPF};(11) 99999-0000;telefone;bureau\n"
                     f"{CPF};5511999990000;telefone;cadastro\n{CPF};x;fax;\n", encoding="utf-8")
        contatos, rej = carregar_carteira(p)
